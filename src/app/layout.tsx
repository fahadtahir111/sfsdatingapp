  ...
  return (
    <html lang="en" suppressHydrationWarning className={`${inter.variable} ${outfit.variable}`}>
      <body
        ...
        className="antialiased bg-background text-foreground font-sans min-h-screen selection:bg-primary/25 selection:text-foreground"
      >
        <AppWrapper initialUser={user ? { id: user.id, name: user.name } : null}>
          ...
          <div className="flex flex-col md:flex-row min-h-screen bg-background overflow-x-hidden">
            <Sidebar />
            <main className="flex-1 w-full relative min-h-[100dvh] md:h-screen md:overflow-y-auto no-scrollbar">
              <div className="w-full max-w-[var(--page-max)] mx-auto min-h-[inherit]">
                {children}
              </div>
            </main>
          </div>
