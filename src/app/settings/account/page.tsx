  ...
  return (
    <div className="min-h-screen bg-background pt-10 px-6 pb-24">
      ...
      <form className="space-y-8" onSubmit={handleUpdate}>
        <div>
          <label className="text-[10px] font-black text-stone-500 uppercase tracking-widest ml-1 mb-2 block">Legal Name</label>
          <input
            type="text"
            value={name}
        ...
        <div>
          <label className="text-[10px] font-black text-stone-500 uppercase tracking-widest ml-1 mb-2 block">Primary Email</label>
          <input
            type="email"
            value={profile?.email || "verified@sfselite.com"}
            disabled
            className="w-full px-6 py-4 bg-white/5 border border-white/5 rounded-2xl text-stone-600 font-bold outline-none cursor-not-allowed"
          />
          <p className="text-[10px] text-stone-600 mt-2 ml-1 font-medium">Email cannot be changed after verification.</p>
        </div>

        <div>
          <label className="text-[10px] font-black text-stone-500 uppercase tracking-widest ml-1 mb-2 block">Account Level</label>
          <div className="w-full px-6 py-4 bg-white/5 border border-white/5 rounded-2xl text-white font-black flex items-center justify-between uppercase tracking-widest text-xs">
            {profile?.membership}
            <span className="text-[10px] px-3 py-1 bg-primary rounded-lg text-black font-black uppercase shadow-lg shadow-primary/20">Active</span>
          </div>
        </div>

        <button 
          disabled={saving}
          className={`w-full py-5 mt-8 text-xs font-black uppercase tracking-[0.2em] rounded-2xl shadow-2xl active:scale-95 transition-all flex justify-center items-center gap-3 ${saved ? 'bg-green-500 text-white shadow-green-500/20' : 'bg-primary text-black shadow-primary/20'}`}
        >
          {saving ? (
